...
def run_dbt_transformations():
  ...
  try:
    ...
    'dbt',
    'run',
    '--profile', BIGQUERY_DATASET_ID_ID,
...
# T#step 1: importing python libraries and modules
import os
import subprocess
from google.cloud import storage, bigquery
...
job_config = bigquery.loadJobconfig(
    ...
    skip_leading_rows= 1,
    autodetect=True,
    write_dosposition=bigquery.WriteDisposition.WRITE_TRUNCATE
) 

#Execute the load job
load_job = bigquery_client.load_table_from_uri(gcs_uri, dataset_ref.table(table_id))
load_job.result()
print(f'Data loaded to bigQuery table {dataset_id}.{table_id}')
...
def run_dbt_transformations():
  ...
  def main():
    try:
        ...
        # 1. Upload the CSV to GCS
        upload_to_gcs(GCS_BUCKET_NAME, LOCAL_CSV_PATH_IN_CONTAINER, GCS_DESTINATION_BLOB_NAME)

        # 2. Load data from GCS into a BigQuery table
        create_or_update_bigquery_table(BIGQUERY_DATASET_ID, BIGQUERY_TABLE_ID, GCS_URI_FOR_BIGQUERY_LOAD)
